...
struct Veg: Identifiable, Codable, Hashable {
    let id: Int
    let name: String
    let price: Double
...
class VegsViewModel: ObservableObject {
    @Published var vegs: [Veg] = []
    ...
    func fetchVegs() {
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            .map(\.data)
            .decode(type: VegResponse.self, decoder: JSONDecoder())
            .replaceError(with: VegResponse(message: "Failed", data: []))
            .receive(on: DispatchQueue.main)
            .sink { [weak self] response in
                self?.vegs = response.data
...
class VegWishlistModel: ObservableObject {
    @Published var items: Set<String> = []

    func toggle(veg: Veg) {
        let vegId = String(veg.id)
        if items.contains(vegId) {
            items.remove(vegId)
        } else {
            items.insert(vegId)
        }
    ...
    func contains(_ veg: Veg) -> Bool {
        items.contains(String(veg.id))
    }
}
...
struct VegPage: View {
    ...
    @StateObject private var wishlist = VegWishlistModel()
    ...
    var body: some View {
        NavigationStack {
            VStack {
                HStack {
                    TextField("Find your favorite veg item...", text: $viewModel.searchQuery, onCommit: {
                        viewModel.fetchVegs()
                    })
                    .padding(10)
                ...
                if viewModel.isLoading {
                ...
                } else if viewModel.vegs.isEmpty {
                    VStack {
                        Image(systemName: "leaf")
                            .font(.system(size: 40))
                            .foregroundColor(.gray)
                        Text("No veg items available.")
                            .font(.headline)
                            .foregroundColor(.gray)
                            .padding(.top, 5)
                    }
                    ...
                    List {
                        ForEach(viewModel.vegs, id: \.id) { veg in
                            VegCard(
                                veg: veg,
                                isInWishlist: wishlist.contains(veg),
                                onAddToCart: { quantity in
                                    let cartItem = CartItem(
                                        id: veg.id,
                                        name: veg.name,
                                        description: veg.description,
                                        price: Double(veg.price),
                                        imageUrl: veg.image_url,
                                        quantity: quantity
                                    )
                                    cart.add(item: cartItem)
                                    viewModel.updatePopularity(foodId: String(veg.id))
                                },
                                onToggleWishlist: {
                                    wishlist.toggle(veg: veg)
                                }
                            )
        ...
        ZStack(alignment: .topLeading) {
            VStack(alignment: .leading, spacing: 8) {
                HStack(alignment: .top, spacing: 12) {
                    // --- Veg Image ---
                    AsyncImage(url: URL(string: veg.image_url)) { phase in
                        switch phase {
                        case .empty:
                            ProgressView()
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        HStack {
                            ...
                            Button(action: {
                                ...
                                Image(systemName: favoriteModel.containsItemId(veg.id, in: category) ? "heart.fill" : "heart")
                                    .foregroundColor(favoriteModel.containsItemId(veg.id, in: category) ? .red : .gray)
                            }
                        }
            ...
            if let discount = veg.discount_percent, discount > 0 {
...
#Preview {
    VegPage(userName: "John Doe", phoneNumber: "1234567890")
